...
struct
    ...
    let
      ...
      and do_stm(T.SEQ(a,b)) =
        ...
        | do_stm(T.JUMP(e,labs)) =
            reorder_stm([e],fn [e] => T.JUMP(e,labs)
                             | l   => (ErrorMsg.error 0 "Canonical tree error!"; T.EXP (T.CONST 0)))
      ...
      and do_exp(T.BINOP(p,a,b)) =
            reorder_exp([a,b], fn [a,b] => T.BINOP(p,a,b)
                                | l     => (ErrorMsg.error 0 "Canonical tree error!"; T.CONST 0))
        | do_exp(T.MEM(a)) =
  ...
  fun basicBlocks stms =
    let
      ...
      fun blocks((head as T.LABEL _) :: tail, blist) =
        let
          fun next((s as (T.JUMP _))::rest, thisblock) =
  ...
  fun splitlast([]) = (ErrorMsg.error 0 "Canonical tree error!"; (nil, T.EXP (T.CONST 0)))
    | splitlast([x]) = (nil,x)
  ...
  fun trace(table,b as (T.LABEL lab :: _),rest) =
    ...
    in
      ...
      | (most, _) => (ErrorMsg.error 0 "Canonical tree error!"; getnext(table, rest))
    end
    | trace (table,_,rest) = (ErrorMsg.error 0 "Canonical tree error!"; getnext(table, rest))

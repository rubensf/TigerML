...
struct
  ...
  and traverseExp(env:escEnv, d:depth, s:A.exp): unit =
    let
      ...
      |   travexp (A.RecordExp {fields, typ}) =
        let
          fun f (nil) = ()
          |   f ((_,exp,_)::rest) = (traverseExp(env,d,arg);f(rest))
        in
          f fields
        end
      ...
      |   traverseExp (A.ArrayExp {typ,size,init,pos}) = (traverseExp(env,d,size);traverseExp(env,d,init))
    in
      travexp s
    end
  ...
  and traverseDecs(env, d, s: A.dec list): escEnv =
    let
      fun f (A.FunctionDec fundecs, env) =
        let
          fun h ({name,escape,typ}, env) = (escape:=false;S.enter(env,name,(d+1,escape)))
          fun g ({name,params,result,body,pos},env) =
            let

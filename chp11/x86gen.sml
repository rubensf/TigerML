structure X86Gen :> CODEGEN where type frame = X86Frame.frame =
struct
  structure A = Assem
  ...
  fun codegen frame stm =
    let
      ...
      fun arthToString T.PLUS    = "add"
        | arthToString T.MINUS   = "sub"
        | arthToString T.MUL     = "imul"
        | arthToString T.AND     = "and"
        | arthToString T.OR      = "or"
        | arthToString T.XOR     = "xor"
        | relToString _    = (ErrorMsg.error 0 "Internal error - this op is done separately."
      ...
      and munchExp(T.CONST i) =
            result(fn r => emit(A.OPER {assem="mov     `d0, " ^ (i2s i) ^ "\n",
                                        src=[],
                                        dst=[r], jump=NONE}))
        | munchExp (T.BINOP(T.PLUS, T.CONST i1, T.CONST i2)) =
            munchExp (T.CONST (Word.toInt (Word.+(Word.fromInt i1, Word.fromInt i2))))
        ...
        | munchExp (T.BINOP(T.DIV, T.CONST i1, T.CONST i2)) =
            munchExp (T.CONST (Word.toInt (Word.div(Word.fromInt i1, Word.fromInt i2))))
        | munchExp (T.BINOP(T.AND, T.CONST i1, T.CONST i2)) =
            munchExp (T.CONST (Word.toInt (Word.andb(Word.fromInt i1, Word.fromInt i2))))
        | munchExp (T.BINOP(T.OR, T.CONST i1, T.CONST i2)) =
            munchExp (T.CONST (Word.toInt (Word.orb(Word.fromInt i1, Word.fromInt i2))))
        | munchExp (T.BINOP(T.XOR, T.CONST i1, T.CONST i2)) =
            munchExp (T.CONST (Word.toInt (Word.xorb(Word.fromInt i1, Word.fromInt i2))))
        ...
        | munchExp (T.BINOP(T.DIV, e, T.CONST i)) =
            result(fn r => emit(
                                A.OPER {assem="div     `d0, `s0, `s1\n",
                                        ...
                                        dst=[r], jump=NONE}))
        | munchExp (T.BINOP(op, e, T.CONST i)) =
            result(fn r => emit(A.MOVE {assem="mov     `d0, `s0\n",
                                        src=(munchExp e)
                                        dst=r};
                           emit(A.OPER {assem="add     `d0, " ^ (i2s i) ^ "\n",
                                        src=[r],
                                        dst=[r], jump=NONE}))
